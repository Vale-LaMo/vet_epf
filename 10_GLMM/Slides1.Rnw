...
\begin{document}
\SweaveOpts{prefix.string=pictures/Rplot}
...
<<dati-prep, echo=FALSE, eval=TRUE>>=

dati.glm <- read.csv("dati/dati.secondolivello3.csv")[,c("id","Egg10","MyrmicaYN","Noperaiepittfall","Horigano", "HGrass", "Origanupatchdiameter","BV","BVR","BR","BF","NB")]
write.csv(dati.glm,"dati/dati.glm.csv")
dati.GLMM <- read.csv("dati/dati.secondolivello3.csv")[,c("id","Specimen","Egg10","MyrmicaYN","Noperaiepittfall","Horigano", "HGrass", "Origanupatchdiameter","BV","BVR","BR","BF","NB")]
write.csv(dati.GLMM,"dati/dati.GLMM.csv")
...
<<label=datiglm-correlazioni, fig=TRUE, include=FALSE, echo=FALSE>>=

pairs(dati.glm[c("Noperaiepittfall","Horigano","HGrass","Origanupatchdiameter")])
# cor(dati.glm[,-c(1,2)],method="spearman")[,c(1:3)]
# cor(dati.glm[,-c(1,2)],method="spearman")>0.5

@


<<label=datiglm-correlazioni2, fig=TRUE, include=FALSE, echo=FALSE>>=
...
<<label=glm-modelfitting, eval=TRUE, results=hide>>=
...
<<label=glm-dredge, echo=FALSE, fig=TRUE, include=FALSE, results=hide>>=
...
<<label=glm-modelselectionplot, echo=FALSE, fig=TRUE, include=FALSE>>=
...
# sull'asse di destra sono riportati i nomi (numeri identificativi dei modelli)
...
<<label=individual-models, eval=TRUE, echo=FALSE>>=

library(MuMIn)
length(unique(dati.GLMM$Specimen))
...
mod.glm2 <- glm(Egg10 ~ Noperaiepittfall + 
                 Horigano + HGrass + 
                 Origanupatchdiameter + 
                 factor(BV) + factor(BVR) + 
                 factor(BF) + factor(NB),
...
<<label=representation, echo=FALSE, eval=TRUE, include=FALSE, fig=TRUE>>=
